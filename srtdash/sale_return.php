            ...
            <div class="main-content-inner"> 
                <div class="row"> 
                    ...
                    <div class="col-12 mt-5"> 
                        <div class="card"> 
                            <div class="card-body"> 
                                <h4 class="header-title">Sale return Master</h4> 
                <form action="sale_return.php" method="POST" enctype="multipart/form-data"> 
                                       <div class="form-row"> 
...
$result = $conn->prepare("insert into salereturn_master(date,cus_id,po_id,taxmaster,taxamtmaster,gstmaster,gstamtmaster,discountmaster,discountamtmaster,grandtotalmaster,sub,payment)values('$date','$name','$po_id','$taxmaster','$taxamtmaster','$gstmaster','$gstamtmaster','$discountmaster','$discountamtmaster','$grandtotalmaster' ,'$sub','$payment')"); 
        $result->execute();   
        if (count($product) > 0) 
    { 
   ...
   { 
     $result2 = $conn->prepare("INSERT INTO salereturn_details(product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ('$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')");    
        $result2->execute();    
       $resultstk = $conn->prepare("INSERT INTO stock_det(stock, product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('$quantity[$i]','$product[$i]',0,0,0,(select max(id) from salereturn_master))"); 
	   $resultstk->execute();
                 ...
                 if($result)    
                    {  
                        ...
                        $result3 = $conn->prepare("select max(id) as id from salereturn_master");    
                            $result3->execute(); 
                            $row3 = $result3->fetch();   
