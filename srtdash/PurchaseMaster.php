...
$result = $conn->prepare("insert into purchase_master(id,date,supplier_id,po_id,taxmaster,taxamtmaster,gstmaster,gstamtmaster,discountmaster,discountamtmaster,grandtotalmaster,sub,payment)values('$id','$date','$name','$po_id','$taxmaster','$taxamtmaster','$gstmaster','$gstamtmaster','$discountmaster','$discountamtmaster','$grandtotalmaster' ,'$sub','$payment')"); 
   ...
   {  
     $result2 = $conn->prepare("INSERT INTO purchase_details(purchase_id,product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ((select max(id) from purchase_master),'$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')"); 
        $result2->execute();    
       $resultstk = $conn->prepare("INSERT INTO stock_det(stock, product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('$quantity[$i]','$product[$i]',(select max(id) from purchase_master),0,0,0)"); 
	   $resultstk->execute();
