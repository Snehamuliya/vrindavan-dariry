...
<script> 
 ...
 <form action="PurchaseMaster.php" method="POST" enctype="multipart/form-data">
 <div class="form-row">
...
red">*</span> </label>
 ...
 <?php
 $result = $conn->prepare("SELECT * From supplier_detail");
 $result->execute();
...
name="date">
 ...
 <th scope="col">Tax %</th>
 <th scope="col">Tax Amt</th>Pro Web Graphics
...
id="quantity1" name="quantity[]" onkeyup="calcu()" style="width: 80px;">Pro Web Graphics
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Price" id="price1" 
name="price[]" onkeyup="calcu()" style="width: 70px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="amount" 
id="amount1" value="0" readonly="" name="amount[]" onkeyup="calcu('newtable')" style="width: 
90px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Tax %" 
name="tax[]" id="tax1" onkeyup="taxcal()" style="width: 70px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Tax Amt" 
name="taxamt[]" id="taxamt1" onkeyup="taxcal('newtable')" readonly="" value="0" style="width: 
90px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Discount %" 
id="discount1" name="discount[]" onkeyup="discal()" style="width: 70px;" >
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Discount Amt" 
id="discountamt1" name="discountamt[]" onkeyup="discal('newtable')" readonly="" value="0" 
style="width: 90px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="GST%" id="gst1" 
name="gst[]" onkeyup="gstcal()" style="width: 70px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="GST Amt" 
id="gstamt1" name="gstamt[]" onkeyup="gstcal('newtable')" readonly="" value="0" style="width: 
90px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Cess%" id="cess1" 
name="cess[]" onkeyup="cesscal()" style="width: 70px;">
 </td>
 <td>
 <input class="form-control" type="text" placeholder="Cess Amt" 
id="cessamt1" name="cessamt[]" onkeyup="cesscal('newtable')" readonly="" value="0" style="width: 
90px;">
 </td>
 <td>
 <input class="form-control" type="number" placeholder="Final Amount" 
id="finalamount1" name="finalamount[]" onkeyup="finalcal('newtable')" readonly="" value="0" 
style="width: 100px;">
 </td>
...
id="sub" name="sub" onkeyup="mastergst()">
 </div>
 </div>
 <div class="form-row" style="display: none;">
...
id="grandtotalmaster" name="grandtotalmaster" onkeyup="mastergrand()" readonly="">
 </div>
 <div class="col-md-2 mb-3">
...
name="submit">Save</button>
 <a href="PurchaseList.php" class="btn btn-primary" id="submit">Back</a>
 </div>
 </div>
...
function taxcalrows(i)
{
 ...
 b = parseFloat(b);Pro Web Graphics
 if(a > 0 && b >0)
{
 var amt=(a * b) / 100;
 amt = parseFloat(amt);
...
function finalcal()
{
 ...
 f = parseFloat(f);
 var t = document.getElementById('taxamt1').value;Pro Web Graphics
 t = parseFloat(t);
...
function rowFunction() {
 ...
 var price = a.insertCell(2);Pro Web Graphics
...
rowCount +"' name='quantity[]' onkeyup='calcurows("+ rowCount +")' >";Pro Web Graphics
...
id='gst"+ rowCount +"' onkeyup='gstcalrows("+ rowCount +")'>";
...
purchase_master(supplier_id,date,taxmaster,taxamtmaster,gstmaster,gstamtmaster,discountmaster,dis
countamtmaster,grandtotalmaster,sub,payment)values('$name','$date','$taxmaster','$taxamtmaster','$
gstmaster','$gstamtmaster','$discountmaster','$discountamtmaster','$grandtotalmaster' 
,'$sub','$payment')");
 $result->execute();
 if (count($product) > 0)
 {
 ...
 //echo $amount[$i]."<br/>";
 // echo $tax[$i]."<br/>";
 //echo $taxamt[$i]."<br/>";Pro Web Graphics
 //echo $discount[$i]."<br/>";
 //echo $discountamt[$i]."<br/>";
 //echo $finalamount[$i]."<br/>";
 {
...
amt,cess,cessamt,finalamount) VALUES ((select max(id) from 
purchase_master),'$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discou
nt[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')");
 $result2->execute(); 
 $resultstk = $conn->prepare("INSERT INTO stock_det(stock, 
product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('-
$quantity[$i]','$product[$i]',0,(select max(id) from purchase_master),0,0)");
 $resultstk->execute(); 
}
}
 if($result) 
 ...
 $result3 = $conn->prepare("select max(id) as id from purchase_master"); 
 $result3->execute();
 $row3 = $result3->fetch();
 echo "<script>
 alert('Your record is inserted successfully...!');
 window.location.href='PurchaseList.php?ID=".$row3['id']."';
 </script>";
 }
 else
...
placeholder="ID" readonly name="id" value="<?php echo $row['id']; ?>">
 ...
 <div class="col-md-3 mb-3">
 <label for="validationCustom01">Supplier<span class="star" style="color: 
red">*</span> </label>Pro Web Graphics
 <select class="form-control" name="name">
 <?php
 
 if($row['supplier_id']>0)
 {
 $result3 = $conn->prepare("SELECT * From supplier_detail where 
id=".$row['supplier_id']."");
 $result3->execute();
...
."</option>" ;
 ...
 $result4 = $conn->prepare("SELECT * From supplier_detail where id not 
in(".$row['supplier_id'].") ");
...
purchase_id=".$id);
 ...
 {
 $result3 = $conn->prepare("SELECT * From product_master where 
...
."</option>" ;
 ...
 $result4 = $conn->prepare("SELECT * From product_master where id 
...
$cessamt=$_POST['cessamt'];
 $finalamount=$_POST['finalamount'];
$result = $conn->prepare("update purchase_master set supplier_id= '$name',date= '$date' ,taxmaster 
='$taxmaster' ,taxamtmaster= '$taxamtmaster' ,gstmaster='$gstmaster' 
,gstamtmaster='$gstamtmaster' ,discountmaster= '$discountmaster',discountamtmaster= Pro Web Graphics
'$discountamtmaster' ,grandtotalmaster= '$grandtotalmaster' ,sub= '$sub', payment='$payment' Where 
ID=$id ");
 $result->execute();
 ...
 for ($i=0;$i<=count($product)-1;$i++){
 // $result2 = $conn->prepare("INSERT INTO 
sale_detail(sale_id,product_id,quantity,price,amount,tax,taxamt,discount,discountamt,finalamount) 
VALUES ((select max(id) from sale_master),'$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]'
,'$discountamt[$i]','$finalamount[$i]')");
...
?>
 ...
 <!-- main content area end -->
<?php include("Footer.php");?>Pro Web Graphics
PurchaseList.php
<?php include("Header.php");?>
 <div class="main-content-inner">
 <div class="row">
...
btn-outline-primary mb-3">Add New</button></a>
 ...
 <tbody>
 <?phpPro Web Graphics
 $result = $conn->prepare("SELECT * FROM purchase_master ORDER BY id 
desc");
 $result->execute();
 ...
 if($row['supplier_id']>0)
 {
 $resultcom = $conn->prepare("SELECT * From supplier_detail where 
id=".$row['supplier_id']."");
 $resultcom->execute();
