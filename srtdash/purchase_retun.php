            ...
            <div class="main-content-inner"> 
                <div class="row"> 
                    ...
                    <div class="col-12 mt-5"> 
                        <div class="card"> 
                            <div class="card-body"> 
                                <h4 class="header-title">Purchase return Master</h4> 
                <form action="purchase_retun.php" method="POST" enctype="multipart/form-data"> 
                                       <div class="form-row"> 
...
$result = $conn->prepare("insert into purchasereturn_master(id,date,supplier_id,po_id,taxmaster,taxamtmaster,gstmaster,gstamtmaster,discountmaster,discountamtmaster,grandtotalmaster,sub,payment)values('$id','$date','$name','$po_id','$taxmaster','$taxamtmaster','$gstmaster','$gstamtmaster','$discountmaster','$discountamtmaster','$grandtotalmaster' ,'$sub','$payment')"); 
        $result->execute();   
        if (count($product) > 0) 
    { 
   ...
   { 
     $result2 = $conn->prepare("INSERT INTO purchasereturn_details(product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ('$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')");    
        $result2->execute();    
       $resultstk = $conn->prepare("INSERT INTO stock_det(stock, product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('-$quantity[$i]','$product[$i]',0,(select max(id) from purchasereturn_master),0,0)"); 
	   $resultstk->execute();
                 ...
                 if($result)    
                    {  
                        ...
                        $result3 = $conn->prepare("select max(id) as id from purchasereturn_master");    
                            $result3->execute(); 
                            $row3 = $result3->fetch();   
