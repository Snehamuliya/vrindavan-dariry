            ...
            <div class="main-content-inner"> 
                <div class="row"> 
                    ...
                    <div class="col-12 mt-5"> 
                        <div class="card"> 
                            <div class="card-body"> 
                                <h4 class="header-title">Purchase Master Edit</h4> 
                <form action="PurchaseEdit.php?ID=<?php echo $id;?>" method="POST" enctype="multipart/form-data"> 
                                       <div class="form-row"> 
                                ...
                                <div class="single-table"> 
                                    <div class="table-responsive datatable-dark"> 
                                        <table class="table table-bordered text-center" id="newtable"> 
                                            ...
                                            <tbody>
 <?php
 $result5 = $conn->prepare("SELECT * FROM purchase_details where purchase_id=".$id);
 $result5->execute();
...
$result1 = $conn->prepare("update purchase_master set date='$date',	supplier_id='$name',gstmaster='$gstmaster',grandtotalmaster='$grandtotalmaster',sub='$sub',payment='$payment' Where id=$id "); 
        $result1->execute();   
        if (count($product) > 0)
    { 
 $result3 = $conn->prepare("delete from purchase_details Where purchase_id=$id"); 
 $result3->execute();
 ...
 $result8->execute(); 
   ...
   {  
     $result2 = $conn->prepare("INSERT INTO purchase_details(purchase_id,product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ('$id','$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')"); 
	 $result2->execute(); 
        $resultstk = $conn->prepare("INSERT INTO stock_det(stock, product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('$quantity[$i]','$product[$i]','$id',0,0,0)"); 
        $resultstk->execute();  
	   ...
	   /* stock value */	   
	    if($result2)    
                    {  
							...
							$result = $conn->prepare("SELECT * FROM purchase_details");
		                                   $result->execute();
		                                  $row = $result->fetch();
        							
								$p_name =$row['product_id'];
								$qty =$row['quantity'];
								$result=$conn->prepare("update stock set quantity='$qty' Where id=$p_name ");
