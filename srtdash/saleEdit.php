...
<script>    
    function rowFunction(x) { 
            ...
            <div class="main-content-inner"> 
                <div class="row"> 
										...
										$id=$_GET["ID"];
											$result = $conn->prepare("SELECT * FROM sale_master where id=".$id);
		                                   $result->execute();
		                                  $row = $result->fetch();
                                           ?>
                ...
                <form action="saleEdit.php?ID=<?php echo $id;?>" method="POST" enctype="multipart/form-data"> 
                                       <div class="form-row"> 
 ...
 if($row['cus_id']>0)
 {
 $result3 = $conn->prepare("SELECT * From customer where 
id=".$row['cus_id']."");
 $result3->execute();
...
."</option>" ;
 ...
 $result4 = $conn->prepare("SELECT * From customer where id not 
in(".$row['cus_id'].") ");
...
."</option>" ;
                                ...
                                <div class="single-table"> 
                                    <div class="table-responsive datatable-dark"> 
                                        <table class="table table-bordered text-center" id="newtable"> 
                                            ...
                                            <tbody>
 <?php
 $result5 = $conn->prepare("SELECT * FROM sale_details where sale_id=".$id);
 $result5->execute();
 ...
 {
 $result3 = $conn->prepare("SELECT * From prod1 where id=".$rowdet['product_id']."");
 $result3->execute();
 $rowcat2 = $result3->fetch();
 echo "<option selected value='". $rowcat2['id'] ."'>" .$rowcat2['name'] ."</option>" ;
 }
 $result4 = $conn->prepare("SELECT * From prod1 where id not in(".$rowdet['product_id'].") ");
 $result4->execute();
 for($k=0; $rowcompall1 = $result4->fetch(); $k++)
 {
...
//$result = $conn->prepare("insert into purchase_master(id,date,supplier_id,po_id,taxmaster,taxamtmaster,gstmaster,gstamtmaster,discountmaster,discountamtmaster,grandtotalmaster,sub,payment)values('$id','$date','$name','$po_id','$taxmaster','$taxamtmaster','$gstmaster','$gstamtmaster','$discountmaster','$discountamtmaster','$grandtotalmaster' ,'$sub','$payment')"); 
$result1 = $conn->prepare("update sale_master set date='$date',cus_id='$name',gstmaster='$gstmaster',grandtotalmaster='$grandtotalmaster',sub='$sub',payment='$payment' Where id=$id "); 
        $result1->execute();   
        if (count($product) > 0)
    { 
 $result3 = $conn->prepare("delete from sale_details Where sale_id=$id"); 
 $result3->execute();
 $result8 = $conn->prepare("delete from stock_det Where sale_id=$id"); 
 $result8->execute(); 
   for ($i=0;$i<=count($product)-1;$i++) 
 
   {  
     //$result2 = $conn->prepare("INSERT INTO purchase_return(product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ('$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')"); 
     $result2 = $conn->prepare("INSERT INTO sale_details(sale_id,product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ('$id','$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')"); 
	 $result2->execute(); 
        $resultstk = $conn->prepare("INSERT INTO stock_det(stock, product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('-$quantity[$i]','$product[$i]',0,0,'$id',0)"); 
        $resultstk->execute();      
} 
