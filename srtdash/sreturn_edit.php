    ...
    function rowFunction(x) { 
            ...
            <div class="main-content-inner"> 
                <div class="row"> 
										...
										$id=$_GET["ID"];
											$result = $conn->prepare("SELECT * FROM salereturn_master where id=".$id);
		                                   $result->execute();
		                                  $row = $result->fetch();
                                           ?>
                    ...
                    <div class="col-12 mt-5"> 
                        <div class="card"> 
                            <div class="card-body"> 
                                <h4 class="header-title">sale edit Master</h4> 
                <form action="sreturn_edit.php?ID=<?php echo $id;?>" method="POST" enctype="multipart/form-data"> 
                                       <div class="form-row"> 
                                ...
                                <div class="single-table"> 
                                    <div class="table-responsive datatable-dark"> 
                                        <table class="table table-bordered text-center" id="newtable"> 
                                            ...
                                            <tbody>
 <?php
 $result5 = $conn->prepare("SELECT * FROM salereturn_details where saleret_id=".$id);
 $result5->execute();
...
value="Credit">Credit</option>
                                     ...
                                     <div class="form-row"> 
                                        ...
                                        <div class="col-md-2 mb-3" > 
                                             ...
                                             <a href="saleView.php" class="btn btn-primary" id="submit">Back</a> 
                                        </div> 
                                    </div> 
...
$result1 = $conn->prepare("update salereturn_master set date='$date',cus_id='$name',gstmaster='$gstmaster',grandtotalmaster='$grandtotalmaster',sub='$sub',payment='$payment' Where id=$id "); 
        $result1->execute();   
        if (count($product) > 0)
    { 
 $result3 = $conn->prepare("delete from salereturn_details Where saleret_id=$id"); 
 $result3->execute();
 $result8 = $conn->prepare("delete from stock_det Where saleret_id=$id"); 
 $result8->execute(); 
   for ($i=0;$i<=count($product)-1;$i++) 
 
   {  
     $result2 = $conn->prepare("INSERT INTO salereturn_details(saleret_id,product_id,quantity,price,amount,tax,taxamt,discount,discountamt,gst,gstamt,cess,cessamt,finalamount) VALUES ('$id','$product[$i]','$quantity[$i]','$price[$i]','$amount[$i]','$tax[$i]','$taxamt[$i]','$discount[$i]','$discountamt[$i]','$gst[$i]','$gstamt[$i]','$cess[$i]','$cessamt[$i]','$finalamount[$i]')"); 
	 $result2->execute(); 
        $resultstk = $conn->prepare("INSERT INTO stock_det(stock, product_id,Purchase_id,PurchaseRet_id,Sale_id,SaleRet_id) VALUES ('$quantity[$i]','$product[$i]',0,0,0,'$id')"); 
        $resultstk->execute();      
} 
                 ...
                 if($result1)    
                    {  
                         ...
                         alert('Your record is Updated successfully...!'); 
                           window.location.href='sreturn_view.php?ID=".$row3['id']."'; 
                             </script>"; 
                        }  
                         else 
